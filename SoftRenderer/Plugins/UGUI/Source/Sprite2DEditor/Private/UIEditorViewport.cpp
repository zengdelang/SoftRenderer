...
SUIEditorViewport::~SUIEditorViewport()
{
	PaperViewportClient.Reset();
}

TSharedRef<FEditorViewportClient> SUIEditorViewport::MakeEditorViewportClient()
{
	return PaperViewportClient.ToSharedRef();
}

void SUIEditorViewport::Construct(const FArguments& InArgs, TSharedRef<FEditorViewportClient> InViewportClient)
{
...
void SUIEditorViewport::Tick( const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime )
{
...
FReply SUIEditorViewport::OnMouseButtonDown(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	TotalMouseDelta = 0;
...
FReply SUIEditorViewport::OnMouseButtonUp(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
...
FReply SUIEditorViewport::OnMouseMove(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
...
FReply SUIEditorViewport::OnMouseWheel(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
...
FCursorReply SUIEditorViewport::OnCursorQuery( const FGeometry& MyGeometry, const FPointerEvent& CursorEvent ) const
{
...
int32 SUIEditorViewport::FindNearestZoomLevel(float InZoomAmount, bool bRoundDown) const
{
...
float SUIEditorViewport::GetZoomAmount() const
{
	return GetZoomLevel(ZoomLevel).ZoomAmount;
}

FText SUIEditorViewport::GetZoomText() const
{
	return GetZoomLevel(ZoomLevel).DisplayText;
}

FSlateColor SUIEditorViewport::GetZoomTextColorAndOpacity() const
{
	return FLinearColor( 1, 1, 1, 1.25f - ZoomLevelFade.GetLerp() * 0.75f );
}

FVector2D SUIEditorViewport::GetViewOffset() const
{
	return ViewOffset;
}

FVector2D SUIEditorViewport::ComputeEdgePanAmount(const FGeometry& MyGeometry, const FVector2D& TargetPosition)
{
...
void SUIEditorViewport::UpdateViewOffset(const FGeometry& MyGeometry, const FVector2D& TargetPosition)
{
	const FVector2D PanAmount = ComputeEdgePanAmount( MyGeometry, TargetPosition ) / GetZoomAmount();
	ViewOffset += PanAmount;
}

void SUIEditorViewport::RequestDeferredPan(const FVector2D& UpdatePosition)
{
	bRequestDeferredPan = true;
	DeferredPanPosition = UpdatePosition;
}

FVector2D SUIEditorViewport::GraphCoordToPanelCoord( const FVector2D& GraphSpaceCoordinate ) const
{
	return (GraphSpaceCoordinate - GetViewOffset()) * GetZoomAmount();
}

FVector2D SUIEditorViewport::PanelCoordToGraphCoord( const FVector2D& PanelSpaceCoordinate ) const
{
	return PanelSpaceCoordinate / GetZoomAmount() + GetViewOffset();
}

FSlateRect SUIEditorViewport::PanelRectToGraphRect( const FSlateRect& PanelSpaceRect ) const
{
...
void SUIEditorViewport::PaintSoftwareCursor(const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, FSlateWindowElementList& OutDrawElements, int32 DrawLayerId) const
{
...
int32 SUIEditorViewport::OnPaint(const FPaintArgs& Args, const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, FSlateWindowElementList& OutDrawElements, int32 LayerId, const FWidgetStyle& InWidgetStyle, bool bParentEnabled) const
{
...
FText SUIEditorViewport::GetTitleText() const
{
