#include "Sprite2DThumbnailRenderer.h"
...
//////////////////////////////////////////////////////////////////////////
// USprite2DThumbnailRenderer

FVector SpriteAxisX(1.0f, 0.0f, 0.0f);
FVector SpriteAxisY(0.0f, 0.0f, 1.0f);
FVector SpriteAxisZ(0.0f, 1.0f, 0.0f);

USprite2DThumbnailRenderer::USprite2DThumbnailRenderer(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
}

void USprite2DThumbnailRenderer::Draw(UObject* Object, int32 X, int32 Y, uint32 Width, uint32 Height, FRenderTarget* RenderTarget, FCanvas* Canvas, bool bAdditionalViewFamily)
{
	USprite2D* Sprite = Cast<USprite2D>(Object);
	DrawFrame(Sprite, X, Y, Width, Height, RenderTarget, Canvas, nullptr);
}

void USprite2DThumbnailRenderer::DrawGrid(int32 X, int32 Y, uint32 Width, uint32 Height, FCanvas* Canvas)
{
...
void USprite2DThumbnailRenderer::DrawFrame(class USprite2D* Sprite, int32 X, int32 Y, uint32 Width, uint32 Height, FRenderTarget*, FCanvas* Canvas, FBoxSphereBounds* OverrideRenderBounds)
{
	const UTexture2D* SourceTexture = nullptr;
	if (Sprite != nullptr)
	{
		SourceTexture = Sprite->GetSourceTexture();
	}
	...
	if (SourceTexture != nullptr)
	{
		...
		Sprite->GetSimpleRenderDataForThumbnail(RenderData, Bounds);
		...
		const TArray<FVector4>& BakedRenderData = RenderData;
		...
		const FVector2D MinPoint(FVector::DotProduct(MinPoint3D, SpriteAxisX), FVector::DotProduct(MinPoint3D, SpriteAxisY));
		const FVector2D MaxPoint(FVector::DotProduct(MaxPoint3D, SpriteAxisX), FVector::DotProduct(MaxPoint3D, SpriteAxisY));
