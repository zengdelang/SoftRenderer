...
TSharedRef<IDetailCustomization> FToggleComponentDetails::MakeInstance()
{
	return MakeShareable(new FToggleComponentDetails);
}

void FToggleComponentDetails::CustomizeDetails(IDetailLayoutBuilder& DetailBuilder)
{
	...
	TargetScriptPtr = Cast<UToggleComponent>(TargetObjects[0].Get());

	IDetailCategoryBuilder& ToggleCategory = DetailBuilder.EditCategory(TEXT("Toggle"), FText(LOCTEXT("ToggleComponentCategory", "Toggle")), ECategoryPriority::Important);

	AddRowHeaderContent(ToggleCategory, DetailBuilder);

	ToggleCategory.AddProperty(TEXT("bInteractable"), USelectableComponent::StaticClass());
	
	ToggleCategory.AddProperty(TEXT("Transition"), USelectableComponent::StaticClass());
	...
	if (TargetScriptPtr->GetTransition() == ESelectableTransition::Transition_ColorTint)
	{
		...
		auto& ColorTintGroup = ToggleCategory.AddGroup(TEXT("ColorTint"), LOCTEXT("ColorSpriteBlock_ColorTint", "ColorTint"), false, true);
	...
	else if (TargetScriptPtr->GetTransition() == ESelectableTransition::Transition_SpriteSwap)
	{
		...
		auto& SpriteGroup = ToggleCategory.AddGroup(TEXT("Sprite"), LOCTEXT("ColorSpriteBlock_Sprite", "Sprite"), false, true);
	...
	else if (TargetScriptPtr->GetTransition() == ESelectableTransition::Transition_ColorTintAndSpriteSwap)
	{
		...
		auto& ColorTintAndSpriteGroup = ToggleCategory.AddGroup(TEXT("ColorTintAndSprite"), LOCTEXT("ColorSpriteBlock_ColorTintAndSprite", "ColorTintAndSprite"), false, true);
	...
	ToggleCategory.AddProperty(TEXT("bIsOn"));
	ToggleCategory.AddProperty(TEXT("ToggleTransition"));

	ToggleCategory.AddProperty(TEXT("BackgroundPath"));
	ToggleCategory.AddProperty(TEXT("CheckMarkPath"));

	AddEventProperty(ToggleCategory, DetailBuilder, TEXT("OnValueChanged"));
}

void FToggleComponentDetails::MakeColorMultiplierRow(IDetailGroup& ColorTintAndSpriteGroup, const TSharedRef<IPropertyHandle> ColorSpriteBlockProperty) const
{
