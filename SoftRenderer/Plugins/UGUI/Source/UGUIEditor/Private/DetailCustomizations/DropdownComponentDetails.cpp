...
TSharedRef<IDetailCustomization> FDropdownComponentDetails::MakeInstance()
{
	return MakeShareable(new FDropdownComponentDetails);
}

void FDropdownComponentDetails::CustomizeDetails(IDetailLayoutBuilder& DetailBuilder)
{
	...
	TargetScriptPtr = Cast<UDropdownComponent>(TargetObjects[0].Get());

	IDetailCategoryBuilder& DropdownCategory = DetailBuilder.EditCategory(TEXT("Dropdown"), FText(LOCTEXT("DropdownComponentCategory", "Dropdown")), ECategoryPriority::Important);

	AddRowHeaderContent(DropdownCategory, DetailBuilder);

	DropdownCategory.AddProperty(TEXT("bInteractable"), USelectableComponent::StaticClass());
	
	DropdownCategory.AddProperty(TEXT("Transition"), USelectableComponent::StaticClass());
	...
	if (TargetScriptPtr->GetTransition() == ESelectableTransition::Transition_ColorTint)
	{
		...
		auto& ColorTintGroup = DropdownCategory.AddGroup(TEXT("ColorTint"), LOCTEXT("ColorSpriteBlock_ColorTint", "ColorTint"), false, true);
	...
	else if (TargetScriptPtr->GetTransition() == ESelectableTransition::Transition_SpriteSwap)
	{
		...
		auto& SpriteGroup = DropdownCategory.AddGroup(TEXT("Sprite"), LOCTEXT("ColorSpriteBlock_Sprite", "Sprite"), false, true);
	...
	else if (TargetScriptPtr->GetTransition() == ESelectableTransition::Transition_ColorTintAndSpriteSwap)
	{
		...
		auto& ColorTintAndSpriteGroup = DropdownCategory.AddGroup(TEXT("ColorTintAndSprite"), LOCTEXT("ColorSpriteBlock_ColorTintAndSprite", "ColorTintAndSprite"), false, true);
	...
	DropdownCategory.AddProperty(TEXT("DropdownItemWidgetTemplateClass"));
	DropdownCategory.AddProperty(TEXT("Options"));

	DropdownCategory.AddProperty(TEXT("CaptionTextPath"));
	DropdownCategory.AddProperty(TEXT("CaptionImagePath"));

	DropdownCategory.AddProperty(TEXT("PlaceholderPath"));
	DropdownCategory.AddProperty(TEXT("DropdownPath"));
	DropdownCategory.AddProperty(TEXT("ScrollContentPath"));

	DropdownCategory.AddProperty(TEXT("ExtraSpace"));
	DropdownCategory.AddProperty(TEXT("Value"));
	DropdownCategory.AddProperty(TEXT("AlphaFadeSpeed"));

	AddEventProperty(DropdownCategory, DetailBuilder, TEXT("OnValueChanged"));
}

void FDropdownComponentDetails::MakeColorMultiplierRow(IDetailGroup& ColorTintAndSpriteGroup, const TSharedRef<IPropertyHandle> ColorSpriteBlockProperty) const
{
