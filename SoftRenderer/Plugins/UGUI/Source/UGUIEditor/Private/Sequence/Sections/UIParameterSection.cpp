...
#include "Sequence/Sections/MovieSceneUISectionHelpers.h"
#include "IKeyArea.h"
#include "EditorStyleSet.h"
#include "Rendering/DrawElements.h"

#define LOCTEXT_NAMESPACE "UIParameterSection"

FReply FUIParameterSection::OnKeyDoubleClicked(const TArray<FKeyHandle> &KeyHandles )
{
	...
	{
		FMovieSceneUIKeyColorPicker KeyColorPicker(ParameterSection, &NameAndCurve.RedCurve, &NameAndCurve.GreenCurve, &NameAndCurve.BlueCurve, &NameAndCurve.AlphaCurve, KeyHandles);
	}

	return FReply::Handled();
}

int32 FUIParameterSection::OnPaintSection(FSequencerSectionPainter& Painter) const
{
	...
	for (FKeyAreaElement KeyAreaElement : Painter.KeyAreaElements)
	{
		...
		if (RChannel && GChannel && BChannel && AChannel)
		{
			...
			if ( GradientSize.X >= 1.f )
			{
				...
				MovieSceneUISectionHelpers::ConsolidateColorCurves( ColorKeys, DefaultColor, ColorChannels, TimeConverter );
...
bool FUIParameterSection::RequestDeleteCategory( const TArray<FName>& CategoryNamePath )
{
...
bool FUIParameterSection::RequestDeleteKeyArea( const TArray<FName>& KeyAreaNamePath )
{
