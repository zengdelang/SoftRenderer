...
class UIBLUEPRINTEDITOR_API FUIChildActorComponentEditorUtils
{
public:
	/** Returns true if the given SCS editor tree node is a child actor node */
	static bool IsChildActorNode(TSharedPtr<const FSCSComponentEditorTreeNode> InNodePtr);

	/** Returns true if the given SCS editor tree node belongs to a child actor template */
	static bool IsChildActorSubtreeNode(TSharedPtr<const FSCSComponentEditorTreeNode> InNodePtr);

	/** Returns true if any array element belongs to a child actor template */
	static bool ContainsChildActorSubtreeNode(const TArray<TSharedPtr<FSCSComponentEditorTreeNode>>& InNodePtrs);

	/** Returns the child actor component root node of a child actor subtree */
	static TSharedPtr<FSCSComponentEditorTreeNode> GetOuterChildActorComponentNode(TSharedPtr<const FSCSComponentEditorTreeNode> InNodePtr);
	...
	static void FillChildActorContextMenuOptions(UToolMenu* Menu, TSharedPtr<const FSCSComponentEditorTreeNode> InNodePtr);
