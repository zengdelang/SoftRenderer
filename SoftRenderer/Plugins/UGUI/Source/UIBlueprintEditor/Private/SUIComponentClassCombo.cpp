...
FString FUIComponentClassComboEntry::GetClassName() const
{
	return ComponentClass != nullptr ? ComponentClass->GetDisplayNameText().ToString() : ComponentName;
}

void FUIComponentClassComboEntry::AddReferencedObjects(FReferenceCollector& Collector)
{
...
void SUIComponentClassCombo::Construct(const FArguments& InArgs)
{
	...
	FUIComponentTypeRegistry::Get().SubscribeToComponentList(ComponentClassList).AddRaw(this, &SUIComponentClassCombo::UpdateComponentClassList);

	UpdateComponentClassList();

	SAssignNew(ComponentClassListView, SListView<FUIComponentClassComboEntryPtr>)
		.ListItemsSource(&FilteredComponentClassList)
		.OnSelectionChanged( this, &SUIComponentClassCombo::OnAddComponentSelectionChanged )
		.OnGenerateRow( this, &SUIComponentClassCombo::GenerateAddComponentRow )
		.SelectionMode(ESelectionMode::Single);

	SAssignNew(SearchBox, SSearchBox)
		.HintText( LOCTEXT( "BlueprintAddComponentSearchBoxHint", "Search Components" ) )
		.OnTextChanged( this, &SUIComponentClassCombo::OnSearchBoxTextChanged )
		.OnTextCommitted( this, &SUIComponentClassCombo::OnSearchBoxTextCommitted );
	...
	[
		...
		SNew(SListViewSelectorDropdownMenu<FUIComponentClassComboEntryPtr>, SearchBox, ComponentClassListView)
		[
			SNew(SBorder)
	...
	.OnComboBoxOpened(this, &SUIComponentClassCombo::ClearSelection);
...
SUIComponentClassCombo::~SUIComponentClassCombo()
{
	FUIComponentTypeRegistry::Get().GetOnComponentTypeListChanged().RemoveAll(this);
}

void SUIComponentClassCombo::ClearSelection()
{
...
void SUIComponentClassCombo::GenerateFilteredComponentList()
{
	...
	else
	{
		...
		FUIComponentClassComboEntryPtr* LastHeadingPtr = nullptr;

		for (int32 ComponentIndex = 0; ComponentIndex < ComponentClassList->Num(); ComponentIndex++)
		{
			FUIComponentClassComboEntryPtr& CurrentEntry = (*ComponentClassList)[ComponentIndex];
		...
		for (FUIComponentClassComboEntryPtr& TestEntry : FilteredComponentClassList)
		{
...
FText SUIComponentClassCombo::GetCurrentSearchString() const
{
	return TextFilter->GetFilterText();
}

void SUIComponentClassCombo::OnSearchBoxTextChanged( const FText& InSearchText )
{
...
void SUIComponentClassCombo::OnSearchBoxTextCommitted(const FText& NewText, ETextCommit::Type CommitInfo)
{
...
void SUIComponentClassCombo::OnAddComponentSelectionChanged( FUIComponentClassComboEntryPtr InItem, ESelectInfo::Type SelectInfo )
{
...
TSharedRef<ITableRow> SUIComponentClassCombo::GenerateAddComponentRow( FUIComponentClassComboEntryPtr Entry, const TSharedRef<STableViewBase> &OwnerTable ) const
{
	...
	else
	{
		...
		return
			...
			[
				...
				[
					SNew(STextBlock)
					.HighlightText(this, &SUIComponentClassCombo::GetCurrentSearchString)
					.Text(this, &SUIComponentClassCombo::GetFriendlyComponentName, Entry)
				]
			];
	}
}

void SUIComponentClassCombo::UpdateComponentClassList()
{
	GenerateFilteredComponentList();
}

FText SUIComponentClassCombo::GetFriendlyComponentName(FUIComponentClassComboEntryPtr Entry) const
{
	// Get a user friendly string from the component name
	FString FriendlyComponentName;

	if( Entry->GetComponentCreateAction() == EUIComponentCreateAction::CreateNewCPPClass )
	{
		FriendlyComponentName = LOCTEXT("NewCPPComponentFriendlyName", "New C++ Component...").ToString();
	}
	else if (Entry->GetComponentCreateAction() == EUIComponentCreateAction::CreateNewBlueprintClass )
	{
...
FString SUIComponentClassCombo::GetSanitizedComponentName(FUIComponentClassComboEntryPtr Entry)
{
	FString DisplayName;
...
TSharedRef<SToolTip> SUIComponentClassCombo::GetComponentToolTip(FUIComponentClassComboEntryPtr Entry) const
{
