...
FKismetUIVariableDragDropAction::FKismetUIVariableDragDropAction()
	: VariableName(NAME_None)
{
}

UBlueprint* FKismetUIVariableDragDropAction::GetSourceBlueprint() const
{
...
void FKismetUIVariableDragDropAction::GetLinksThatWillBreak(	UEdGraphNode* Node, FProperty* NewVariableProperty, 
...
void FKismetUIVariableDragDropAction::HoverTargetChanged()
{
	...
	{
		FMyUIBlueprintItemDragDropAction::HoverTargetChanged();
	}
}

void FKismetUIVariableDragDropAction::GetDefaultStatusSymbol(const FSlateBrush*& PrimaryBrushOut, FSlateColor& IconColorOut, FSlateBrush const*& SecondaryBrushOut, FSlateColor& SecondaryColorOut) const
{
...
FReply FKismetUIVariableDragDropAction::DroppedOnPin(FVector2D ScreenPosition, FVector2D GraphPosition)
{
...
FReply FKismetUIVariableDragDropAction::DroppedOnNode(FVector2D ScreenPosition, FVector2D GraphPosition)
{
...
void FKismetUIVariableDragDropAction::MakeGetter(FNodeConstructionParams InParams)
{
...
void FKismetUIVariableDragDropAction::MakeSetter(FNodeConstructionParams InParams)
{
	check(InParams.Graph);
...
bool FKismetUIVariableDragDropAction::CanExecuteMakeSetter(FNodeConstructionParams InParams, FProperty* InVariableProperty)
{
...
FReply FKismetUIVariableDragDropAction::DroppedOnPanel( const TSharedRef< SWidget >& Panel, FVector2D ScreenPosition, FVector2D GraphPosition, UEdGraph& Graph)
{	
	FProperty* VariableProperty = GetVariableProperty();
		...
		if (VariableProperty && CanVariableBeDropped(VariableProperty, Graph))
		{
			...
			else
			{
				...
				MenuBuilder.AddMenuEntry(
					...
					FSlateIcon(),
					FUIAction(
					FExecuteAction::CreateStatic(&FKismetUIVariableDragDropAction::MakeGetter, NewNodeParams), FCanExecuteAction())
					);
				...
				MenuBuilder.AddMenuEntry(
					...
					FSlateIcon(),
					FUIAction(
					FExecuteAction::CreateStatic(&FKismetUIVariableDragDropAction::MakeSetter, NewNodeParams),
					FCanExecuteAction::CreateStatic(&FKismetUIVariableDragDropAction::CanExecuteMakeSetter, NewNodeParams, VariableProperty ))
					);
...
bool FKismetUIVariableDragDropAction::CanVariableBeDropped(const FProperty* InVariableProperty, const UEdGraph& InGraph) const
{
...
UStruct* FKismetUIVariableDragDropAction::GetLocalVariableScope() const
{
