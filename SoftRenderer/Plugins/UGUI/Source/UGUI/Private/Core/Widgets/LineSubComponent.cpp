...
ULineSubComponent::ULineSubComponent(const FObjectInitializer& ObjectInitializer)
...
DECLARE_CYCLE_STAT(TEXT("UIGraphic --- Line::OnPopulateMesh"), STAT_UnrealGUI_Sub_LineOnPopulateMesh,STATGROUP_UnrealGUI);
void ULineSubComponent::OnPopulateMesh(FVertexHelper& VertexHelper)
{
	SCOPE_CYCLE_COUNTER(STAT_UnrealGUI_Sub_LineOnPopulateMesh);
...
void ULineSubComponent::OnEnable()
{
	Super::OnEnable();
...
void ULineSubComponent::OnDisable()
{
	if (const auto Sprite2D = Cast<USprite2D>(Sprite))
	{
...
void ULineSubComponent::SetSprite(UObject* InSprite)
{
