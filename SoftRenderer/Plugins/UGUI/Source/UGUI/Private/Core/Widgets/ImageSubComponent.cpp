...
UImageSubComponent::UImageSubComponent(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	...
	AlphaHitTestMinimumThreshold = 0;
...
float UImageSubComponent::GetPixelsPerUnit()
{
...
void UImageSubComponent::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	const FName MemberPropertyName = PropertyChangedEvent.MemberProperty ? PropertyChangedEvent.MemberProperty->GetFName() : FName();
	if (MemberPropertyName == GET_MEMBER_NAME_CHECKED(UImageSubComponent, FillMethod))
	{
		FillOrigin = 0;
		OriginHorizontal = EOriginHorizontal::OriginHorizontal_Left;
		OriginVertical = EOriginVertical::OriginVertical_Bottom;
	...
	else if (MemberPropertyName == GET_MEMBER_NAME_CHECKED(UImageSubComponent, OriginHorizontal) && FillMethod == EFillMethod::FillMethod_Horizontal)
	{
		FillOrigin = static_cast<int32>(OriginHorizontal);
	}
	else if (MemberPropertyName == GET_MEMBER_NAME_CHECKED(UImageSubComponent, OriginVertical) && FillMethod == EFillMethod::FillMethod_Vertical)
	{
		FillOrigin = static_cast<int32>(OriginVertical);
	}
	else if (MemberPropertyName == GET_MEMBER_NAME_CHECKED(UImageSubComponent, Origin90) && FillMethod == EFillMethod::FillMethod_Radial90)
	{
		FillOrigin = static_cast<int32>(Origin90);
	}
	else if (MemberPropertyName == GET_MEMBER_NAME_CHECKED(UImageSubComponent, Origin180) && FillMethod == EFillMethod::FillMethod_Radial180)
	{
		FillOrigin = static_cast<int32>(Origin180);
	}
	else if (MemberPropertyName == GET_MEMBER_NAME_CHECKED(UImageSubComponent, Origin360) && FillMethod == EFillMethod::FillMethod_Radial360)
	{
...
void UImageSubComponent::OnEnable()
{
	Super::OnEnable();
...
void UImageSubComponent::OnDisable()
{
	if (const auto Sprite2D = Cast<USprite2D>(Sprite))
	{
...
float UImageSubComponent::GetPreferredWidth()
{
	const auto ActiveSprite = GetActiveSprite();
...
float UImageSubComponent::GetPreferredHeight()
{
	const auto ActiveSprite = GetActiveSprite();
	if (!IsValid(ActiveSprite))
		return 0;
...
DECLARE_CYCLE_STAT(TEXT("UIGraphic --- IsRaycastLocationValid"), STAT_UnrealGUI_Sub_ImageRaycast, STATGROUP_UnrealGUI);
bool UImageSubComponent::IsRaycastLocationValid(IMaskableGraphicElementInterface* MaskableGraphicElement, const FVector& WorldRayOrigin, const FVector& WorldRayDir,
	bool bIgnoreReversedGraphicsScreenPoint)
{
	SCOPE_CYCLE_COUNTER(STAT_UnrealGUI_Sub_ImageRaycast);
	...
	if (!FRectTransformUtility::ScreenPointToLocalPointInRectangle(AttachTransform, WorldRayOrigin, WorldRayDir, LocalPosition))
		return false;
	
	return true;
}

DECLARE_CYCLE_STAT(TEXT("UIGraphic --- OnPopulateMesh"), STAT_UnrealGUI_Sub_ImageOnPopulateMesh, STATGROUP_UnrealGUI);
void UImageSubComponent::OnPopulateMesh(FVertexHelper& VertexHelper)
{
    SCOPE_CYCLE_COUNTER(STAT_UnrealGUI_Sub_ImageOnPopulateMesh);
...
FVector2D UImageSubComponent::GetNativeSize()
{
	const auto ActiveSprite = GetActiveSprite();
	if (const auto PaperSprite = Cast<UPaperSprite>(ActiveSprite))
	{
...
void UImageSubComponent::PreserveSpriteAspectRatio(FRect& ViewRect, const FVector2D& SpriteSize) const
{
	...
	if (SpriteRatio > RectRatio)
	{
		...
		if (bFillViewRect)
		{
			...
			ViewRect.XMin += (OldWidth - ViewRect.Width) * AttachTransform->GetPivot().X;
		}
		else
		{
			const float OldHeight = ViewRect.Height;
			ViewRect.Height = ViewRect.Width * (1.0f / SpriteRatio);
			ViewRect.YMin += (OldHeight - ViewRect.Height) * AttachTransform->GetPivot().Y;
		}
	}
	else
	{
...
FVector4 UImageSubComponent::GetDrawingDimensions(bool bShouldPreserveAspect, FRect& OriginalViewRect)
{
	const auto ActiveSprite = GetActiveSprite();
...
void UImageSubComponent::GenerateSimpleSprite(FVertexHelper& VertexHelper, bool bInPreserveAspect)
{
...
void UImageSubComponent::GenerateSimpleSpriteAA(FVertexHelper& VertexHelper, bool bInPreserveAspect)
{
	FRect OriginalViewRect;
	const FVector4 ViewRect = GetDrawingDimensions(bInPreserveAspect, OriginalViewRect);
	...
	else
	{
		...
		VertexHelper.AddVert(FVector(ViewRect.X, ViewRect.Y, 0), Color, FVector2D(OuterUV.X, OuterUV.W), UV1, FVector2D(-1, -1));
		VertexHelper.AddVert(FVector(ViewRect.X, ViewRect.W, 0), Color, FVector2D(OuterUV.X, OuterUV.Y), UV1, FVector2D(-1, 1));
		VertexHelper.AddVert(FVector(ViewRect.Z, ViewRect.W, 0), Color, FVector2D(OuterUV.Z, OuterUV.Y), UV1, FVector2D(1, 1));
		VertexHelper.AddVert(FVector(ViewRect.Z, ViewRect.Y, 0), Color, FVector2D(OuterUV.Z, OuterUV.W), UV1, FVector2D(1, -1));
...
void UImageSubComponent::GenerateSlicedSprite(FVertexHelper& VertexHelper)
{
	if (!HasBorder())
	{
		if (!bAntiAliasing)
	...
	const auto& Pivot = AttachTransform->GetPivot();
...
FVector4 UImageSubComponent::GetAdjustedBorders(FVector4 InBorder, const FRect& InAdjustedRect) const
{
	const FRect& OriginalRect = AttachTransform->GetRect();
...
void UImageSubComponent::GenerateTiledSprite(FVertexHelper& VertexHelper)
{
	const auto ActiveSprite = GetActiveSprite();
	if (!IsValid(ActiveSprite))
		return;
	...
	FRect Rect = GetPixelAdjustedRect();
	...
	SpriteBorder = GetAdjustedBorders(SpriteBorder / MultipliedPixelsPerUnit(), Rect);

	FVector2D UVMin = FVector2D(InnerUV.X, InnerUV.W);
	FVector2D UVMax = FVector2D(InnerUV.Z, InnerUV.Y);

	FVector2D CenterUV2Min = FVector2D(-1 + TileWidth/Rect.Width, -1 + TileHeight/Rect.Height);
	FVector2D CenterUV2Max = FVector2D(1 - TileWidth/Rect.Width, 1 - TileHeight/Rect.Height);
	
	// Min to max max range for tiled region in coordinates relative to lower left corner.
	float XMin = SpriteBorder.X;
	float XMax = Rect.Width - SpriteBorder.Z;
	float YMin = SpriteBorder.Y;
	float YMax = Rect.Height - SpriteBorder.X;
	...
	if (HasBorder())
	{
		...
		for (int64 H = 0; H < NTilesH; ++H)
		{
			...
			if (bFillCenter)
			{
				...
				AddQuad(VertexHelper,
					...
					FVector2D(Rect.Width, Y2) + RectPosition,
					Color,
		...
		for (int64 W = 0; W < NTilesW; ++W)
		{
			...
			if (bFillCenter)
			{
				...
				AddQuad(VertexHelper,
					...
					FVector2D(X2, Rect.Height) + RectPosition,
					Color,
		...
		AddQuad(VertexHelper,
			...
			FVector2D(UVMin.X, UVMin.Y), UV1, FVector2D(-1, -1), CenterUV2Min, ImageMaskType, UV5, Rect);
		AddQuad(VertexHelper,
			FVector2D(XMax, 0) + RectPosition,
			FVector2D(Rect.Width, YMin) + RectPosition,
			Color,
			FVector2D(UVMax.X, OuterUV.Y),
			FVector2D(OuterUV.Z, UVMin.Y), UV1, FVector2D(CenterUV2Max.X, -1), FVector2D(1, CenterUV2Min.Y), ImageMaskType, UV5, Rect);
		AddQuad(VertexHelper,
			FVector2D(0, YMax) + RectPosition,
			FVector2D(XMin, Rect.Height) + RectPosition,
			Color,
			FVector2D(OuterUV.X, UVMax.Y),
			FVector2D(UVMin.X, OuterUV.W), UV1, FVector2D(-1, CenterUV2Max.Y), FVector2D(CenterUV2Min.X, 1), ImageMaskType, UV5, Rect);
		AddQuad(VertexHelper,
			FVector2D(XMax, YMax) + RectPosition,
			FVector2D(Rect.Width, Rect.Height) + RectPosition,
			Color,
			FVector2D(UVMax.X, UVMax.Y),
			FVector2D(OuterUV.Z, OuterUV.W), UV1, FVector2D(CenterUV2Max.X, CenterUV2Max.Y), FVector2D(1, 1), ImageMaskType, UV5, Rect);
	}
}

void UImageSubComponent::GenerateFilledSprite(FVertexHelper& VertexHelper, bool bInPreserveAspect)
{
	VertexHelper.Empty();
