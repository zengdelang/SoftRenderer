...
UTextSubComponent::UTextSubComponent(const FObjectInitializer& ObjectInitializer)
...
void UTextSubComponent::SetText(FText InText)
{
    if (InText.IsEmpty())
    {
        if (Text.IsEmpty())
...
void UTextSubComponent::Awake()
{
	...
	if (IsValid(AttachTransform))
	{
		...
		{
			Widget = Cast<URectTransformComponent>(AttachTransform->GetChildComponent(Elem.Value.ChildWidgetIndex));
			if (IsValid(Widget))
			{
				...
				//FDelegateHandle AttachWidgetRectChangeDelegateHandle = Widget->OnRectTransformDimensionsChangeEvent.AddUObject(this, &UTextSubComponent::OnAttachWidgetRectTransformDimensionsChange);
...
void UTextSubComponent::OnEnable()
{
    Super::OnEnable();
...
void UTextSubComponent::OnDisable()
{
    TextCache.Reset();
...
void UTextSubComponent::OnDestroy()
{
...
void UTextSubComponent::UpdateMaterial()
{
...
void UTextSubComponent::UpdateGeometry()
{
	if (IsValid(Font))
	{
...
FTextGenerationSettings UTextSubComponent::GetGenerationSettings(FVector2D Extents)
{
    ...
    Settings.Pivot = AttachTransform->GetPivot();
...
DECLARE_CYCLE_STAT(TEXT("UIGraphic --- Text::OnPopulateMesh"), STAT_UnrealGUI_Sub_TextOnPopulateMesh, STATGROUP_UnrealGUI);
void UTextSubComponent::OnPopulateMesh(FVertexHelper& VertexHelper)
{
	SCOPE_CYCLE_COUNTER(STAT_UnrealGUI_Sub_TextOnPopulateMesh);
    ...
    CachedTextGenerator.Populate(Text.ToString(), GetGenerationSettings(AttachTransform->GetRect().GetSize()), this);
	...
	for (auto& WidgetRegion : WidgetRegionList)
	{
		...
		{
			Widget->WidgetRectTransform->AttachToComponent(AttachTransform, FAttachmentTransformRules::KeepRelativeTransform);
	...
	if (IsValid(World))
	{
		if (bAnimation)
		{
			...
			if (!TimerHandle.IsValid())
			{
				World->GetTimerManager().SetTimer(TimerHandle,this, &UTextSubComponent::TickEmojiAnimation, 0.001f, false);
			}
		}
		else
		{
...
float UTextSubComponent::GetPreferredWidth()
{
    return GetCachedTextGeneratorForLayout().GetPreferredWidth(Text.ToString(), GetGenerationSettings(FVector2D::ZeroVector), this);
}

float UTextSubComponent::GetPreferredHeight()
{
    return GetCachedTextGeneratorForLayout().GetPreferredHeight(Text.ToString(), GetGenerationSettings(FVector2D(AttachTransform->GetRect().GetSize().X, 0)), this);
}

FTextEmoji* UTextSubComponent::GetTextEmoji(FName InEmojiSymbol)
{
...
FTextWidget* UTextSubComponent::GetTextWidget(FName InWidgetSymbol)
{
...
FAttachWidget* UTextSubComponent::GetAttachWidget(FName InWidgetSymbol)
{
...
UTextEmojiImageComponent* UTextSubComponent::GenerateEmojiImageComponent() const
{
	UTextEmojiImageComponent* EmojiImageComponent = NewObject<UTextEmojiImageComponent>(AttachTransform->GetOwner(), UTextEmojiImageComponent::StaticClass(), NAME_None, RF_Transient);
	...
	EmojiImageComponent->AttachToComponent(AttachTransform, FAttachmentTransformRules::KeepRelativeTransform);
	EmojiImageComponent->SetLocalTransform(FTransform::Identity);

	EmojiImageComponent->SetAnchorAndOffsetAndPivot(FVector2D(0, 0), FVector2D(1, 1), FVector2D(0, 0), FVector2D(0, 0), AttachTransform->GetPivot());
	EmojiImageComponent->AwakeFromLoad();
	return EmojiImageComponent;
}

void UTextSubComponent::ClearEmojis()
{
	CurEmojiList.Empty();
...
void UTextSubComponent::ResetEmojis()
{
	for (const auto& Elem : UsedEmojiImages)
	{
		if (Elem.Value.IsValid())
...
void UTextSubComponent::TickEmojiAnimation()
{
	const auto World = GetWorld();
	if (IsValid(World))
	{
...
void UTextSubComponent::UpdateEmojis(bool bOnlyUpdateAnimation, float DeltaTime)
{
...
UTextEmojiImageComponent* UTextSubComponent::GetValidEmojiImageComponent(UTexture* InTexture)
{
...
void UTextSubComponent::OnAttachWidgetRectTransformDimensionsChange()
{
