...
#include "LayoutGroupSubComponent.generated.h"

/**
 * Abstract base class to use for layout groups.
 */
UCLASS(Abstract, Blueprintable, BlueprintType, meta = (DisallowMultipleComponent))
class UGUI_API ULayoutGroupSubComponent : public UBehaviourSubComponent, public ILayoutElementInterface, public ILayoutGroupInterface
{
...
protected:
    ...
    FORCEINLINE float GetStartOffset(int32 Axis, float RequiredSpaceWithoutPadding)
    {
        ...
        const float AvailableSpace = AttachTransform->GetRect().GetSize()[Axis];
   ...
   /**
    ...
    FORCEINLINE bool IsRootLayoutGroup() const
    {
        ...
        const auto LayoutParent = AttachTransform->GetAttachParent();
    ...
    FORCEINLINE void SetDirty()
    {
        ...
        else
        {
            ...
            {
                EndFrameDelegateHandle = FCoreDelegates::OnEndFrame.AddUObject(this, &ULayoutGroupSubComponent::DelayedSetDirty);
            }
        }
    }
